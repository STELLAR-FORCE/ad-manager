...
- [x] ダッシュボード CPA/CV 推移チャートに「日別／累積」モードトグル追加（Issue #37）
  - CPA: 累積 cost ÷ 累積 CV で滑らかなトレンド、CV: プラットフォーム別 stacked AreaChart
  - URL query param + localStorage で状態永続化
- [x] サイドバー折りたたみの a11y / モーション改善（Issue #21）
  - `prefers-reduced-motion` 尊重、ラベル translate-x でスライド感強化
  - 共通フック `hooks/use-prefers-reduced-motion.ts` を作成
- [x] 指標セルに hover tooltip を追加（Issue #22）
  - CTR / CPC / CVR / CPA の分子・分母を内訳表示
  - `components/ui/metric-tooltip.tsx` を新規作成
  - `TooltipProvider delay={200}` を MainLayout に配置
- [x] 広告グループ一覧のフィルターをキャンペーンと統一（Issue #26 / #27）
- [x] Google Ads 風レイアウトへの再構築（Issue #41）
  - `/ad-groups`: lg: 以上で 検索 / その他 を横並び、それ未満で縦積み
  - `/campaigns`: 検索 / その他 の 2 セクション構造に再構築
  - `/campaigns/[id]`: 単一セクションで KPI + チャート追加
  - `/campaigns/[id]/[adGroupId]`: タブ（広告 / キーワード / クリエイティブ）に応じてチャートの折れ線が切替
  - 共通コンポーネント群を `components/ad-insights/` に集約
    - `kpi-strip.tsx`・`kpi-tile.tsx`（6 タイル固定 + 「指標を追加」で CTR/CVR/CPA 追加可能）
    - `trend-chart.tsx`（Recharts LineChart、LINE_COLORS 循環）
    - `metric-defs.ts`（10種のメトリクス定義・PLATFORM_PALETTES）
  - モック日次トレンド生成 `lib/trend-mock.ts`（シード固定で決定論的）
  - テーブルのチェックボックスで選択した行のみチャート表示（初期: クリック数上位 3 件）
  - 各セクションヘッダーに媒体 / ステータスフィルター（統一）
  - StatusChip: 一時停止を赤（danger）に変更
...
### UI 再構築の残件
- [ ] `/ads` と `/keywords` も Google Ads 風レイアウトに揃える（Issue #43）
- [ ] テーブルを TanStack Table 基盤に再構築（Issue #36）
...
### その他の残 Issue
- [ ] Dependabot 脆弱性 12 件対応（Issue #35・high:4 / medium:8）
- [ ] 検索語句ページ新規作成（Issue #10）
- [ ] 広告全データを BigQuery 担当者に共有（Issue #6）

### 数字検証・データ整合性（HIGH）
- [ ] **mart 行展開によるリード集計ダブルカウント修正**（Issue #97）
  - 6 ファイル分の SQL を `GROUP BY リードID` パターンへ書き換え
  - 進捗カード / cv-based / move-in / leads / summary 全体に影響
- [ ] 各ページの数値ソース確認（Issue #94 — 業務側で進める検証タスク）
- [ ] 入居日 / 発生日で目標値を分ける（Issue #93 — targets_monthly に axis カラム追加）
- [ ] 成約数目標バー復活（Issue #84 — won_target カラム追加）
