...
function Section({ title, adType, period }: SectionProps) {
  ...
  const { rows, totals } = useMemo(() => {
    const filtered = CAMPAIGNS.filter((c) => {
      if (c.adType !== adType) return false;
      if (platform !== 'all' && c.platform !== platform) return false;
      if (statusFilter === 'active') {
        if (c.status !== 'active' && c.status !== 'active_limited') return false;
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-md border">
          <Table>
              ...
              <TableRow>
                <TableHead className="w-8 pl-3">
                  <input
                    ...
                    onChange={(e) => {
                      setSelectedIds(e.target.checked ? new Set(sortedRows.map((r) => r.id)) : new Set());
                    }}
                  />
                </TableHead>
                <TableHead className="w-6" />
                <SortHeader col="name" label="キャンペーン" sort={sort} onSort={toggleSort} className="min-w-44" />
                <TableHead>媒体</TableHead>
                <TableHead>タイプ</TableHead>
                <SortHeader col="dailyBudget" label="日予算" sort={sort} onSort={toggleSort} className="text-right" />
                <SortHeader col="optimizationScore" label="最適化" sort={sort} onSort={toggleSort} className="text-right" />
            ...
            <TableBody>
              {sortedRows.map((c: CampaignData) => {
                const platformCfg = PLATFORM_CONFIG[c.platform];
                ...
                return (
                  <TableRow key={c.id} className="text-sm">
                    ...
                    <TableCell className="text-muted-foreground whitespace-nowrap text-xs">{c.type}</TableCell>
